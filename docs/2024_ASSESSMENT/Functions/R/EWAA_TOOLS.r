...
weight_at_age_wide <- function(dat,
  ...
  if(min(dat$Age_yrs)> 0){
    add_0<-data.table(Source='Fishery',Weight_kg=0.012,Sex='M',Age_yrs=0,Length_cm=15,Month=1,Year=2007) 
    dat=rbind(dat,add_0)
   }
...
makeimage <- function(agevec = 0:10, yrvec = 1990:2022,
  ...
  image(
    x = agevec, y = yrvec2, z = mat2, axes = FALSE, xlab = "Age", ylab = "Year",
...
dointerpSimple <- function(df, skipcols = 1:6) {
  nco<-ncol(df)
  if(!is.numeric(df[,nco])){df<-df[,-nco]}
...
write_wtatage_file <- function(file = paste0("wtatage_", format(Sys.time(), "%Y"), "created_", format(Sys.time(), "%d-%b-%Y_%H.%M"), ".ss"),
  ...
  header <- c(
    "# empirical weight-at-age Stock Synthesis input file for EBS PCOD",
  ...
  fleetn2 <- cbind(
    data[, c(1:6)],
    t(apply((data[fleet==0][, -c(1:6)]), 1, function(x) x * maturity))
  )
  fleetn2$fleet <- -2
  printdf(fleetn2)
