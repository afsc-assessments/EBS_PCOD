...
## Add the old catch (pre-1991) from csv file   
  if(exists("OLD_SEAS_GEAR_CATCH")){
      ...
      CATCH <- bind_rows(OLD_GEAR_CATCH, CATCH_TOTAL) %>%
      arrange(GEAR, YEAR)
  }else{
...
## check that all gears have the same number of years covered; add zeros if necessary
  ...
  CATCH <- CATCH %>%
    ...
    mutate(TONS = if_else(is.na(TONS), 0.0, TONS))
...
## For 2024 base models only a single fleet is used
  ...
  if(!ONE_FLEET){ 
    CATCH <- CATCH %>%
      mutate(GEAR = if_else(GEAR == "OTHER", "LONGLINE", GEAR)) %>%
      group_by(GEAR, YEAR) %>%
      summarise(TONS = sum(TONS)) %>%
...
## ----- Get Fishery catch at size composition data -----
  ...
  if(ONE_FLEET){
        ...
        FISHLCOMP <- FISHLCOMP[[1]]

        Nsamp <- FISHLCOMP %>%
            group_by(YEAR) %>%
            summarise(nsamp = max(NHAUL))

        lcomp <- FISHLCOMP %>%
            group_by(BIN, YEAR) %>%
            summarise(TOTAL = sum(FREQ)) %>%
            left_join(Nsamp, by = "YEAR") %>%
            mutate(TOTAL = TOTAL / nsamp)

        FISHLCOMP <- lcomp %>%
            mutate(across(starts_with("BIN"), ~ if_else(is.na(.), 0, .))) %>%
            pivot_wider(names_from = BIN, values_from = TOTAL, names_prefix = "BIN_") %>%
            select(-nsamp) %>%
            rename_with(~ gsub("BIN_", "", .), starts_with("BIN_"))

        FISHLCOMP <- FISHLCOMP %>%
            mutate(Year = YEAR, Seas = 1, FltSrv = 1, Gender = 0, Part = 0, Nsamp = round(Nsamp$nsamp / (mean(Nsamp$nsamp) / mean(SRV_LCOMP_SS$Nsamp))))
  ...
  if(!ONE_FLEET){ 
    ...
    lcomp <- FSH_LCOMP %>%
        group_by(BIN, GEAR, YEAR) %>%
        summarise(TOTAL = sum(FREQ)) %>%
        left_join(Nsamp, by = c("GEAR", "YEAR")) %>%
        mutate(TOTAL = TOTAL / nsamp)
    ...
    mutate(data = map(data, ~ {
        ...
        x[, 4] <- 0
        x[, 5] <- 0
        x[, 6] <- Nsamp$nsamp[Nsamp$GEAR1 == first(.x$GEAR1)]
    
        for (i in 1:nyr) {
            x[i, 1] <- years[i]
            x[i, 3] <- first(.x$GEAR1)
            x[i, 7:(nbin + 6)] <- .x$TOTAL[.x$YEAR == years[i]]
        }
        x
    })) %>%
        unnest(data) %>%
        rename_with(~ ifelse(. == "GEAR1", "FltSrv", .), GEAR1) %>%
        select(-nsamp)

    FISHLCOMP <- FISHLCOMP %>%
        mutate(Year = Year, Seas = 1, Gender = 0, Part = 0, Nsamp = round(Nsamp$nsamp / (mean(Nsamp$nsamp) / mean(SRV_LCOMP_SS$Nsamp))))
...
#  if(VAST){
     ...
     ACOMP<-data.frame(YEAR=agecompV$Year,month=7,fleet=2,sex=0,part=0,ageerr=1,lbin_lo=1,lbin_hi=120,Nsamp=data.table(SRV_LCOMP_SS)[Year%in%years]$Nsamp,agecompV[,c(2:(vmax_age+2))])
