...
## pulling in LL survey RPN data from AKFIN.
  if(LL_DAT){ 
	...
	LL_RPN=sql_run(akfin, bsrpn) %>% 
      ...
      #d = ggplot(LL_CPUE[index>0],aes(x=year,y=obs))+geom_point()+geom_line()
      #d = d+geom_ribbon(data=LL_CPUE[index>0],aes(ymin=(obs-1.96*SE),ymax=(obs+1.96*SE)),alpha=0.3)
      #d = d+theme_bw(base_size=16)+labs(x='Year',y='Longline survey RPN')+ylim(0,260000)
      #print(d)
...
#   }
      ...
      CAAL<-cond_length_age_cor(max_age1=12)
