...
VAST_abundance_old<-data.table(VAST_abundance_old)[Fleet=="Both"]
...
db_EEBS= readLines('sql/BS_early_srv_biom.sql')
db_EEBS = sql_filter(sql_precode = "IN", x = srv_sp_str, sql_code = db_EEBS, flag = '-- insert species')
db_EEBS=sql_run(afsc, db_EEBS) %>% 
         dplyr::rename_all(toupper)

db_EBS<- rbind(db_EEBS,db_EBS)         
...
TSURVEYP3$FISHERY<-(TSURVEYP3$FISHERY-mean(TSURVEYP3[!is.na(FISHERY)]$FISHERY))/sd(TSURVEYP3[!is.na(FISHERY)]$FISHERY)
