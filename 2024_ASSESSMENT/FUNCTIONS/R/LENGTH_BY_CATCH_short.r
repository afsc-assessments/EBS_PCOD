...
## pull domestic length and species composition data for haul data  
  ...
  mutate(QUARTER = floor((as.numeric(MONTH) / 3) - 0.3) + 1)
...
## pull foreign length and species composition data for haul data
  ...
  mutate(MONTH = as.numeric(MONTH),
         QUARTER = floor((MONTH / 3) - 0.3) + 1)


  Tspcomp <- rbindlist(list(Fspcomp, Dspcomp))
  Tspcomp[, WED := WED(HDAY)]
  Tspcomp[, MONTH_WED := month(WED)]
  ...
  ## This is done by year, area, gear, month, year, gear, quarter, year, area, month, year, area, quarter, year, gear, year, area, year, gear
  ...
  cols_to_convert <- c("YEAR", "MONTH", "QUARTER")
  cols_to_convert1 <- c("YEAR", "MONTH")
  cols_to_convert2 <- c("YEAR", "QUARTER")

  Tspcomp[, (cols_to_convert) := lapply(.SD, as.numeric), .SDcols = cols_to_convert]

  YAGM_AVWT[, (cols_to_convert1) := lapply(.SD, as.numeric), .SDcols = cols_to_convert1]
  YAM_AVWT[, (cols_to_convert1) := lapply(.SD, as.numeric), .SDcols = cols_to_convert1]
  YGM_AVWT[, (cols_to_convert1) := lapply(.SD, as.numeric), .SDcols = cols_to_convert1]
  YAQ_AVWT[, (cols_to_convert2) := lapply(.SD, as.numeric), .SDcols = cols_to_convert2]
  YGQ_AVWT[, (cols_to_convert2) := lapply(.SD, as.numeric), .SDcols = cols_to_convert2]
...
## pull domestic length and composition for port data 1990-1998
  ...
  PADspcomp[, MONTH := as.numeric(MONTH)]
  PADspcomp[, QUARTER := floor((MONTH / 3) - 0.3) + 1]

  PADspcomp[, AREA2 := trunc(AREA / 10) * 10]
  PADspcomp[AREA2 == 500, AREA2 := 510]
...
## calculating average weight for a series of nested levels with the hierarchy of which one is used explained below 
  ...
  PADspcomp2[, AVEWT := YAGM_AVE_WT]
  PADspcomp2[is.na(AVEWT), AVEWT := YAM_AVE_WT]
  PADspcomp2[is.na(AVEWT), AVEWT := YAQ_AVE_WT]
  PADspcomp2[is.na(AVEWT), AVEWT := YGM_AVE_WT]
  PADspcomp2[is.na(AVEWT), AVEWT := YGQ_AVE_WT]
  PADspcomp2[is.na(AVEWT), AVEWT := YG_AVE_WT]
...
## pull domestic length and composition for port data 1999-2007
  ...
  PBFTCKT2 <- data.table(PBFTCKT)[, .(TONS_LANDED = sum(TONS), AREA_NUMBER = length(REPORTING_AREA_CODE), ALL_AREA = paste(unique(REPORTING_AREA_CODE), collapse = ",")), 
                              by = .(AKFIN_SPECIES_CODE, AKFIN_YEAR, DELIVERY_DATE, WEEK_END_DATE, SPECIES_NAME, VESSEL_ID, FMP_SUBAREA, FMP_GEAR, VES_AKR_CG_NUM, VES_AKR_NAME, DELIVERING_VESSEL)]
  ...
  PBCOMB5[, AVEWT := YAGM_AVE_WT]
  PBCOMB5[is.na(AVEWT), AVEWT := YAM_AVE_WT]
  PBCOMB5[is.na(AVEWT), AVEWT := YGQ_AVE_WT]
  PBCOMB5[is.na(AVEWT), AVEWT := YGM_AVE_WT]
  PBCOMB5[is.na(AVEWT), AVEWT := YGQ_AVE_WT]
  PBCOMB5[is.na(AVEWT), AVEWT := YG_AVE_WT]

  PBCOMB5 <- PBCOMB5[order(DELIVERY_DATE, CRUISE, DELIVERING_VESSEL, HAUL_JOIN, SPECIES, SEX, LENGTH)]
  PBCOMB4 <- data.frame(PBCOMB4[order(DELIVERY_DATE, CRUISE, DELIVERING_VESSEL, HAUL_JOIN, SPECIES, SEX, LENGTH)])
