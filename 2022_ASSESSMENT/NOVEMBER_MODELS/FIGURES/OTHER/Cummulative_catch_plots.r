...
Plot_CUMMULATIVE<-function(username1=username,password1=password,species="'PCOD'",FMP_AREA="'BSAI'",subarea="'BS'",syear=2015)
 {
  library(RODBC)
  library(data.table)
  library(ggplot2)
  ...
  cYear<-year(Sys.time())
  nyr<-length(unique(W1$YEAR))

  d<- ggplot(data=W1,aes(x=WEEK,y=CUM,color=factor(YEAR),shape=factor(YEAR),size=factor(YEAR)))
  ...
  d<-d+labs(title=paste0(species," Area = ",FMP_AREA," Subarea = ",subarea), y="Cummulative Catch (t)",shape="Year",size="Year",color="Year")
  d<-d+scale_color_manual(values=c(2:(nyr),"black"))+scale_size_manual(values=c(rep(0.25,nyr-1),1))
  print(d)
}
